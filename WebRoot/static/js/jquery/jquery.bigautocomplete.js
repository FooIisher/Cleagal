	...
	var bigAutocomplete = new function(){
		...
		this.autocomplete = function(param){
			...
			$this.keyup(function(event) {
				...
				if(!isFunctionalKey && (!ctrl || (ctrl && k == 67) || (ctrl && k == 88)) ){
					...
					if(data != null && $.isArray(data) ){
						...
						setTimeout(function(){
							var _b=$.trim($(node).val());
							if (keyword_==_b && keyword_){
			...
			function makeContAndShow(data_){
				...
				//如果找不到商品的时候
				if (data_.length==1 && data_[0].goods_num==0 ){
					 //弹出提示
					var htmlStr ='<div class="mr300"><p class="tip_none">'+data_[0].title+'</p></div>'
					$bigAutocompleteContent.html(htmlStr);
				    $bigAutocompleteContent.show();
				    return;
				}
				var cont = "<table><tbody>";
				for(var i=0;i<data_.length;i++){
					cont += "<tr><td><div style='padding:0px;width:280px;overflow: hidden;text-overflow: ellipsis;white-space: nowrap;'>" + data_[i].title + "</div><span>约" + data_[i].goods_num + "个商品</span></td></tr>"
				}
				cont += "</tbody></table>";

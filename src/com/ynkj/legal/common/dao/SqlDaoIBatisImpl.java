...
public class SqlDaoIBatisImpl extends SqlMapClientTemplate implements SqlDao {
     ...
     */
    private SqlMapClientImpl getSqlMapClientImpl() {
    ...
    public int count(String sqlID, Map params, boolean useConfig) throws DataAccessException {
        ...
        if (useConfig) {
            sqlPrint(sqlID, parameter);
            Object obj = queryForObject(sqlID, parameter);
            if(obj == null){
            	return 0;
            }
    ...
    public int count(String sql) throws DataAccessException {
        return (Integer) super.queryForObject("ynkj.Common.queryCount", sql);
    }
    ...
    public void execute(String sql) throws DataAccessException {
        super.update("ynkj.Common.execute",sql);
    }
    ...
    public List select(String sql) throws DataAccessException {
        return super.queryForList("ynkj.Common.select", sql);
    }
		...
		try{
			 if (list != null ){
				 ...
				 for ( int i = 0, n = list.size(); i < n; i++) { 
                    ...
                    getSqlMapClient().update(statementName, list.get(i)); 
		...
		try{
			 if (list != null ){
				 ...
				 for ( int i = 0, n = list.size(); i < n; i++) { 
                   ...
                   getSqlMapClient().delete(statementName, list.get(i)); 
	...
	public Object[] getObject(int pageNum, int pageCount, List li) {
		...
		int totalRows = li.size();// 总记录数
		int prePages = pageCount;// 每页显示记录数
		int prePageStartNum = 0;// 记录起始数
		if (pageNum != 1) {
			prePageStartNum = (pageNum - 1) * pageCount;
		}
		int prePageEndNum = 0;// 记录结束数
		prePageEndNum = pageNum * pageCount;
		if (prePageEndNum > li.size()){
			prePageEndNum = li.size();
		}
		// 总页数
		int totalPages = (int) Math.ceil((totalRows - 1) / prePages) + 1;
		if (totalPages - pageNum < 0) {
			return null;
		} else {
			for (int i = prePageStartNum; i < prePageEndNum; i++) {
				if (null != li.get(i)) {
					list.add(li.get(i));
				}
			}
			object[0] = pageNum + "";// 第几页
			object[1] = totalPages + ""; // 共几页
			object[2] = list; // 内容（list）
			return object;
		}
	}
}
